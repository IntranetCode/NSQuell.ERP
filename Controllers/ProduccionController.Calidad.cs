  ...
  AND e.Activo=1;";
               ...
               solicitudProduccionId.Value <= 0)
            {
                throw new InvalidOperationException(
                    "No existe una OF relacionada con esta ejecuci�n. Genera la OF desde Planeaci�n antes de enviarla a Calidad.");
            }
            if (!solicitudProduccionDetalleId.HasValue ||
               solicitudProduccionDetalleId.Value <= 0)
            {
                throw new InvalidOperationException(
                    "La OF relacionada no tiene un detalle v�lido asociado al programa.");
            }
...
-- Si ya est� en monitoreo, la acci�n fue ejecutada previamente.
...
-- Generar las horas restantes conforme a la programaci�n.
-- Cuando la fecha programada ya venci�, se generan nueve periodos
-- para continuar el seguimiento de una producci�n atrasada.
...
-- L�mite de seguridad para evitar una generaci�n accidental excesiva.
...
(
    ...
    GETDATE()
        ...
        {
            if (ejecucion == null) throw new ArgumentNullException(nameof(ejecucion), "No se recibi� la ejecuci�n de producci�n.");
            if (vm == null) throw new ArgumentNullException(nameof(vm), "No se recibi� la captura horaria.");
            if (ejecucion.EjecucionProduccionID <= 0) throw new InvalidOperationException("La ejecuci�n de producci�n no es v�lida.");
            if (registroHoraId <= 0) throw new InvalidOperationException("El registro horario no es v�lido.");
            if (usuarioId <= 0) throw new InvalidOperationException("No se pudo identificar al usuario que registr� la producci�n.");
...
IF @InspeccionID IS NULL
    THROW 51050,'No existe una inspecci�n activa de Calidad para la ejecuci�n.',1;

IF @EstadoInspeccion<>N'MONITOREO_ACTIVO'
    THROW 51051,'La inspecci�n de Calidad no se encuentra en monitoreo activo.',1;
...
IF @MonitoreoID IS NULL
    THROW 51052,'No existe un monitoreo horario pendiente para vincular la captura de Producci�n.',1;

IF @RegistroHoraVinculado IS NOT NULL AND @RegistroHoraVinculado<>@RegistroHoraID
    THROW 51053,'El monitoreo seleccionado ya est� vinculado con otra captura horaria.',1;
...
(
    ...
    THROW 51054,'La captura horaria ya est� vinculada con otro monitoreo de Calidad.',1;
...
IF @@ROWCOUNT<>1
    THROW 51055,'El monitoreo cambi� de estado mientras se vinculaba la captura horaria.',1;
...
BEGIN
    ...
    SET @Comentario=CONCAT(
        N'Seguimiento autom�tico generado desde Producci�n. RegistroHoraID: ',
        @RegistroHoraID,
        N'. Periodo: ',
        ...
        @CantidadScrap,
        N' pieza(s) como scrap operativo. Calidad debe confirmar la disposici�n final.'
    );
...
SELECT
    ...
    CONCAT(
        N'Calidad recibi� la captura horaria de Producci�n. RegistroHoraID: ',
        @RegistroHoraID,
        N'. Periodo: ',
        CONVERT(NVARCHAR(19),@FechaHoraInicio,120),
        N' a ',
